...
test("日付からスケジュール追加", () => {
  const list = new EventDateListPickUp([]);
  ...
  expect(list1.value).toEqual(["2023/04/15"]);
  expect(list2.value).toEqual(["2023/04/15", "2023/04/16"]);
});
...
test("同じ日付のスケジュールは消える", () => {
  ...
  expect(list1.length()).toBe(1);
  expect(list1.value).toEqual(["2023/04/16"]);
});
...
test("日付順にソートされる", () => {
  ...
  const list3 = list
    ...
    .toggleByDate(new Date("2023/04/16"));
  expect(list3.value).toEqual(["2023/04/14", "2023/04/15", "2023/04/16"]);
});
